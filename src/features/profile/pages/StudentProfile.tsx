...
import type { StudentProfile } from '../../../shared/contexts/AuthContext';

export default function StudentProfile() {
  const { user } = useAuth();
  const sp = (user?.studentProfile ?? {}) as StudentProfile;
      ...
      <div className="mt-6 rounded-lg border border-slate-200 bg-white p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" defaultValue={sp.university ?? ''} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Course of Study</label>
            <input className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" defaultValue={sp.courseOfStudy ?? ''} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Year of Study</label>
            <input type="number" min={1} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" defaultValue={sp.yearOfStudy ?? undefined} />
          </div>
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-slate-700">Learning Goals</label>
            <textarea className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" rows={4} defaultValue={sp.learningGoals ?? ''} />
          </div>
        </div>
