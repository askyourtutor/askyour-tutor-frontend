      ...
      <div className="mt-6 rounded-lg border border-slate-200 bg-white p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" defaultValue={(user as any)?.profile?.firstName ?? ''} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Last Name</label>
            <input className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" defaultValue={(user as any)?.profile?.lastName ?? ''} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Professional Title</label>
            <input className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" defaultValue={(user as any)?.profile?.professionalTitle ?? ''} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">University / Institution</label>
            <input className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" defaultValue={(user as any)?.profile?.university ?? ''} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Teaching Experience (years)</label>
            <input type="number" min={0} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" defaultValue={(user as any)?.profile?.teachingExperience ?? 0} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Hourly Rate (USD)</label>
            <input type="number" min={0} step="1" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" defaultValue={(user as any)?.profile?.hourlyRate ?? 0} />
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-slate-700">Qualifications (JSON or comma-separated for now)</label>
            <textarea className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" rows={3} defaultValue={(user as any)?.profile?.qualifications ?? ''} />
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-slate-700">Subject Specializations (JSON or comma-separated)</label>
            <textarea className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" rows={3} defaultValue={(user as any)?.profile?.subjectSpecializations ?? ''} />
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-slate-700">Bio</label>
            <textarea className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" rows={5} defaultValue={(user as any)?.profile?.bio ?? ''} />
          </div>
          <div className="md:col-span-2">
